...
func (h *HTTPSender) Send(ctx context.Context, msg Message) error {
	...
	h.body.Reset()
	_, _ = h.body.Write(payload)
	if h.URL == "" || h.Mode == "mock" {
		if h.Media != nil {
			return h.Media.InsertMockDelivery(ctx, h.Name, h.body.String(), clock.Now())
		}
		return nil
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, h.URL, bytes.NewReader(h.body.Bytes()))
	if err != nil {
		return err
	}
	...
	if resp.StatusCode >= 500 || resp.StatusCode == 429 {
		return fmt.Errorf("%w: %d", domain.ErrTransient, resp.StatusCode)
	}
