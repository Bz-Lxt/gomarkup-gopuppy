...
func Sniff(head []byte) (string, error) {
	...
	if bytes.Equal(probe[:8], []byte{0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A}) {
		return "image/png", nil
	}
	if bytes.Equal(probe[:4], []byte("RIFF")) && bytes.Equal(probe[8:12], []byte("WEBP")) {
		return "image/webp", nil
	}
	if bytes.Equal(probe[:4], []byte("%PDF")) {
